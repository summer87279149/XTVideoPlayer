...
static const CGFloat BUTTON_WIDXT   = 48.0f;
static const CGFloat BUTTON_HEIGHT  = 30.0;
static const CGFloat ICON_WIDXT     = 18.0f;
...
#define LEFT_SHRINK     CGRectMake(ICON_WIDXT, self.midY, 0.f, BUTTON_HEIGHT)
#define RIGHT_SHRINK    CGRectMake(ICON_WIDXT + BUTTON_WIDXT, 0, 0.f, BUTTON_HEIGHT)
#define MIDDLE_EXPANDED CGRectMake(ICON_WIDXT, self.midY, BUTTON_WIDXT, BUTTON_HEIGHT)

@interface XTFlashControl ()
...
@implementation XTFlashControl

- (id)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:CGRectMake(0.0f, 0.0f, ICON_WIDXT + BUTTON_WIDXT, BUTTON_HEIGHT)];
    if (self) {
        [self setupView];
    }
    return self;
...
- (void)setupView {
    ...
    _expandedWidth = ICON_WIDXT + (BUTTON_WIDXT * self.labels.count);
...
- (NSArray *)buildLabels:(NSArray *)labelStrings {
    CGFloat x = ICON_WIDXT;
    BOOL first = YES;
    NSMutableArray *labels = [NSMutableArray array];
    for (NSString *string in labelStrings) {
        CGRect frame = CGRectMake(x, self.midY, BUTTON_WIDXT, BUTTON_HEIGHT);
    ...
    if (!self.expanded) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            for (NSUInteger i = 0; i < self.labels.count; i++) {
                ...
                label.frame = CGRectMake(ICON_WIDXT + (i * BUTTON_WIDXT), self.midY, BUTTON_WIDXT, BUTTON_HEIGHT);
                if (i > 0) {
